...
final class ToggleServiceTests: XCTestCase {
    ...
    func testSwitchToDeepSeekFailsWithoutAPIKey() async {
        ...
        do {
            ...
            XCTFail("expected missingAPIKey(.deepSeek)")
        } catch let error as ToggleError {
            guard case .missingAPIKey(.deepSeek) = error else {
    ...
    func testSwitchToDeepSeekThenClaudeRoundTrip() async throws {
        ...
        let on = try await service.switchTo(provider: .deepSeek)
        XCTAssertEqual(on.mode, .deepSeek)
        XCTAssertEqual(on.provider, .deepSeek)
    ...
    func testSetModelSavesPreferenceWithoutRunningProxy() async throws {
        let service = await makeService()
        try await service.setModel("deepseek/deepseek-v4-flash", for: .deepSeek)
        XCTAssertEqual(service.preferences().providerModels[.deepSeek], "deepseek/deepseek-v4-flash")
        XCTAssertEqual(service.proxy.status, .stopped)
    }

    func testSetModelRestartsRunningProxyWithNewModel() async throws {
        guard keychain.write("sk-test-deepseek", for: .deepSeekAPIKey) else {
            throw XCTSkip("Keychain write unavailable in this environment")
        }
    ...
    func testUpdatePortRestartsRunningProxyWithNewBaseURL() async throws {
        guard keychain.write("sk-test-deepseek", for: .deepSeekAPIKey) else {
            throw XCTSkip("Keychain write unavailable in this environment")
        }
        ...
        try await service.updatePort(4010)
    ...
    func testUpdatePortSavesWithoutRunningProxy() async throws {
        let service = await makeService()
        try await service.updatePort(4020)
        XCTAssertEqual(service.preferences().proxyPort, 4020)
        XCTAssertEqual(service.proxy.status, .stopped)
    }

    func testSetModelForInactiveProviderDoesNotActivateOrRestart() async throws {
        guard keychain.write("sk-test-deepseek", for: .deepSeekAPIKey) else {
            throw XCTSkip("Keychain write unavailable in this environment")
        }
        ...
        try await service.setModel("anthropic/claude-opus-4-8-20251101", for: Provider.anthropic)

        XCTAssertEqual(service.preferences().activeProvider, .deepSeek)
        XCTAssertEqual(service.preferences().providerModels[.anthropic], "anthropic/claude-opus-4-8-20251101")
        let config = try String(contentsOf: tempDir.appendingPathComponent("litellm-config.yaml"), encoding: .utf8)
        XCTAssertFalse(config.contains("anthropic/"))
    }

    func testSetAPIKeyStoresAndRetrieves() async {
        let service = await makeService(keychainOverride: keychain)
        service.setAPIKey("sk-test-anthropic", for: Provider.anthropic)
        XCTAssertTrue(service.hasAPIKey(for: Provider.anthropic))
        XCTAssertEqual(service.apiKey(for: Provider.anthropic), "sk-test-anthropic")
        service.setAPIKey("", for: Provider.anthropic)
        XCTAssertFalse(service.hasAPIKey(for: Provider.anthropic))
    }

    // MARK: - Helpers

    private func makeService(healthAlwaysOK: Bool = true, keychainOverride: KeychainStore? = nil) async -> ToggleService {
